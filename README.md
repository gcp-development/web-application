...
The [React version 18](https://reactjs.org/versions) was used to build this user interface(UI). Because of the [concurrent mode](https://reactjs.org/blog/2022/03/29/react-v18.html#what-is-concurrent-react) that comes with this version. This new feature allows React to work on several state updates concurrently. 
 ...
 [TanStack Query](https://tanstack.com/query/latest) providing asynchronous state management, server-state utilities and data fetching and [React Router](https://reactrouter.com/en/main), which allow our app to update the URL from a link click without making another request for another document from the server.

Also,were implemented two "must have features" in every modern UI.
